...
@st.cache_resource
def load_model():
    # Pastikan nama file .pkl sesuai dengan file model churn Anda
...
# Fitur Kategorikal (Diubah sesuai kebutuhan/encoding model Anda)
gender = st.sidebar.selectbox("Gender", ["Male", "Female"])
country = st.sidebar.selectbox("Country", ["USA", "Canada", "UK", "Germany", "France"]) # Sesuaikan pilihan negara
subscription_type = st.sidebar.selectbox("Subscription Type", ["Free", "Basic", "Premium"]) # Sesuaikan tipe langganan
device_type = st.sidebar.selectbox("Device Type", ["Mobile", "Desktop", "Tablet"]) # Sesuaikan tipe perangkat
...
if st.button("Prediksi Status Churn"):
    # CONTOH MAPPING KATEGORIKAL (Sesuaikan dengan metode Encoding yang Anda pakai saat training model):
    # Gender: Male=1, Female=0 (contoh saja)
    gender_enc = 1 if gender == "Male" else 0
    
    # Masukkan semua fitur sesuai urutan kolom X yang digunakan saat training model
    # (Catatan: Pastikan urutan dan preprocessing sesuai dengan skrip Colab Anda)
    features = np.array([[
        gender_enc, 
        age, 
        listening_time, 
    ...
    # TARGET NAMES (Sesuai kolom is_churned: 0 = Tidak Churn, 1 = Churn)
